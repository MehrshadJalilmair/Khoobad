...
public class SCLAlertView: UIViewController {
    ...
    override public func viewDidAppear(animated: Bool) {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(SCLAlertView.keyboardWillShow(_:)), name:UIKeyboardWillHideNotification, object: nil);
    }
...
public class SCLAlertView: UIViewController {
    ...
    public func addButton(title:String, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, showDurationStatus:Bool=false, action:()->Void)->SCLButton {
        ...
        btn.addTarget(self, action:"buttonTapDown:", forControlEvents:[.TouchDown, .TouchDragEnter])
        btn.addTarget(self, action:"buttonRelease:", forControlEvents:[.TouchUpInside, .TouchUpOutside, .TouchCancel, .TouchDragOutside] )
        return btn
    }
    ...
    public func showTitle(title: String, subTitle: String, duration: NSTimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .TopToBottom) -> SCLAlertViewResponder {
        ...
        // Done button
        if appearance.showCloseButton {
            addButton(completeText ?? "Done", target:self, selector:"hideView")
        }
        ...
        if duration > 0 {
            self.duration = duration
            durationTimer?.invalidate()
            durationTimer = NSTimer.scheduledTimerWithTimeInterval(self.duration, target: self, selector:"hideView" , userInfo: nil, repeats: false)
            durationStatusTimer?.invalidate()
            durationStatusTimer = NSTimer.scheduledTimerWithTimeInterval(1.0, target: self, selector:"updateDurationStatus" , userInfo: nil, repeats: true)
        }
